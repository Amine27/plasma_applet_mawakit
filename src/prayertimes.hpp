...
*/
	...
	enum CalculationMethod
	{
        ...
        Fazilet,     // Fazilet Calendar Fazilet
        TPRA,       // Turkish Presidency of Religious Affairs
...
/* -------------------- Interface Functions -------------------- */
    ...
    , is_ramdan(is_ramdan)
	{
		...
		method_params[Makkah]  = MethodConfig(18.5, true,  0.0, true,  90.0);	// Makkah NOTE: fajr was 19 degrees before 1430 hijri
		method_params[Egypt]   = MethodConfig(19.5, true,  0.0, false, 17.5);	// Egypt
        method_params[UOIF]    = MethodConfig(12.0, true,  5.0, false, 12.0);	// France
        method_params[JAKIM]   = MethodConfig(20.0, true,  0.0, false, 18.0);	// Malaysia
        //method_params[Jafari]  = MethodConfig(16.0, false, 4.0, false, 14.0);	// Jafari
        //method_params[Tehran]  = MethodConfig(17.7, false, 4.5, false, 14.0);	// Tehran
        method_params[Fazilet]  = MethodConfig(19.0, true,  7.0, false, 17.0);	// Fazilet
        method_params[TPRA]    = MethodConfig(18.0, true,  10.0, false, 17.0);	// TPRA
		method_params[Custom]  = MethodConfig(18.0, true,  0.0, false, 17.0);	// Custom
	}

	/* return prayer times for a given date */
        void get_prayer_times(int year, int month, int day, double _latitude, double _longitude, double _timezone, bool _dst, double times[])
	{
		latitude = _latitude;
		longitude = _longitude;
                timezone = _dst ? _timezone+1 : _timezone;
		julian_date = get_julian_date(year, month, day) - longitude / (double) (15 * 24);
		compute_day_times(times);
	}

	/* return prayer times for a given date */
        void get_prayer_times(time_t date, double latitude, double longitude, double timezone, bool dst, double times[])
	{
		tm* t = localtime(&date);
                get_prayer_times(1900 + t->tm_year, t->tm_mon + 1, t->tm_mday, latitude, longitude, timezone, dst, times);
	}
    ...
    /* are we in ramadan month */
    void set_ramadan(bool ramdan)
    {
        is_ramdan = ramdan;
    }

	/* get hours and minutes parts of a float time */
	static void get_float_time_parts(double time, int& hours, int& minutes, int &seconds)
	{
	...
	static std::string float_time_to_time24(double time)
	{
		...
		get_float_time_parts(time, hours, minutes, seconds);
		return two_digits_format(hours) + ':' + two_digits_format(minutes) + ':' + two_digits_format(seconds);
	}
	...
	static std::string float_time_to_time12(double time, bool no_suffix = false)
	{
		...
		return int_to_string(hours) + ':' + two_digits_format(minutes) + ':' + two_digits_format(seconds) + (no_suffix ? "" : suffix);
	}
	...
	/* get Qibla direction from True North */
        static double get_qibla_direction(double latitude, double longitude)
        {
                double BM = deg2rad(21.423333); // Qibla Latitude
                double LM = deg2rad(39.823333); // Qibla Longitude
                double L = deg2rad(longitude);
                double B = deg2rad(latitude);
                return rad2deg(atan2(sin(LM-L), (cos(B)*tan(BM)-sin(B)*cos(LM-L))));
        }

        /* get the distance to Qibla in the given unit */
        static double get_qibla_distance(double latitude, double longitude, Unit unit=Kilometers)
        {
                double BM = deg2rad(21.423333); // Qibla Latitude
                double LM = deg2rad(39.823333); // Qibla Longitude
                double L = deg2rad(longitude);
                ...
                return acos(sin(BM) * sin(B) + cos(BM) * cos(B) * cos(L-LM)) * R;
        }
...
/* ---------------------- Compute Prayer Times ----------------------- */
	...
	void adjust_times(double times[])
	{
        ...
        if(calc_method == UOIF)
        {
                times[Fajr] = times[Fajr] - 5.0 / 60.0;
                times[Dhuhr] = times[Dhuhr] + 5.0 / 60.0;
                times[Isha] = times[Isha] + 5.0 / 60.0;
        }
        else if(calc_method == Fazilet)
        {
                times[Fajr] = times[Fajr] - 10.0 / 60.0;
                times[Sunrise] = times[Sunrise] - 5.0 / 60.0;
                times[Dhuhr] = times[Dhuhr] + 10.0 / 60.0;
                times[Asr] = times[Asr] + 10.0 / 60.0;
                times[Isha] = times[Isha] + 10.0 / 60.0;
        }
        else if(calc_method == TPRA)
        {
                times[Fajr] = times[Fajr] - 2.0 / 60.0;
                times[Sunrise] = times[Sunrise] - 7.0 / 60.0;
                times[Dhuhr] = times[Dhuhr] + 8.0 / 60.0;
                times[Asr] = times[Asr] + 5.0 / 60.0;
                times[Isha] = times[Isha] + 3.0 / 60.0;
        }
        else if(calc_method == Makkah)
        {
                if (is_ramdan)		// Isha is 180min after Maghrib in Ramadan
                    times[Isha] = times[Maghrib] + 120.0 / 60.0;
        }
        /*else if(calc_method == Jafari)
        {
                //times[Isha] = (times[Fajr] - times[Sunset]) / 2;
                times[Isha] = time_diff(times[Sunset], times[Fajr])/ 2;
        }*/
        /*else if(calc_method == JAKIM)
        {
                times[Fajr] = times[Fajr] + 5.0 / 60.0;
                times[Dhuhr] = times[Dhuhr] + 5.0 / 60.0;
                times[Asr] = times[Asr] + 5.0 / 60.0;
                times[Maghrib] = times[Maghrib] + 5.0 / 60.0;
                times[Isha] = times[Isha] + 5.0 / 60.0;
        }*/
	}
...
/* ---------------------- Private Variables -------------------- */
    ...
    bool is_ramdan;		// is ramadan month
